...
class OntologyQueries:
    def __init__(self, ontology_path="ontology/space.ttl", use_graphdb=False, graphdb_url="http://localhost:7200/repositories/space"):
        self.g = Graph()
    ...
    def get_instances_of_class(self, class_uri):
        ...
        try:
            class_uri = URIRef(class_uri) if not isinstance(class_uri, URIRef) else class_uri
    ...
    def get_instance_properties(self, instance_uri):
        ...
        try:
            instance_uri = URIRef(instance_uri) if not isinstance(instance_uri, URIRef) else instance_uri
                ...
                for var in qres.vars:
                    ...
                    if value:
                        if isinstance(value, URIRef):
                            ...
                            result[var] = value.value if hasattr(value, 'value') else str(value)
                    else:
                        result[var] = None
                results.append(result)
            
            return {"success": True, "results": results, "variables": [str(var) for var in qres.vars]}
        except Exception as e:
    ...
    def get_graph_data(self):
        ...
        try:
            ...
            instance_count = 0
            for s, p, o in self.g.triples((None, RDF.type, None)):
                if str(o) != "http://www.w3.org/2002/07/owl#Class" and \
                   str(o) != "http://www.w3.org/2002/07/owl#ObjectProperty" and \
            ...
            for s, p, o in self.g.triples((None, None, None)):
                if isinstance(s, URIRef) and isinstance(o, URIRef) and \
                   p != RDF.type and str(p) != "http://www.w3.org/2000/01/rdf-schema#subClassOf":
                    source_id = str(s)
                    target_id = str(o)
